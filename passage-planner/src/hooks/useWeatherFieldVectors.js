...
export function useWeatherFieldVectors(time) {
  ...
  const weatherFieldVector = useMemo(() => {
    ...
    return weatherForecastAtTime
      .filter((f) => (f.forecast?.ForecastWindSpeed || 0) < 1000)
      .map((weather) => ({
        lon: weather.lon,
        lat: weather.lat,
        vector: [
          (weather.forecast?.ForecastWindSpeed || 0) *
            -Math.sin(
              ((weather.forecast?.ForecastWindDirection || 0) / 180) * Math.PI
            ),
          (weather.forecast?.ForecastWindSpeed || 0) *
            Math.cos(
              ((weather.forecast?.ForecastWindDirection || 0) / 180) * Math.PI
