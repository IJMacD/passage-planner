 ...
 * @param {import("../Layers/VectorFieldLayer.js").Field} field
 * @param {number} alpha
 * @param {number} rangeLimit
 * @param {number} scale
 */
function calculateGradient (context, pxWidth, pxHeight, field, alpha, rangeLimit, scale) {
...
addEventListener("message", (ev) => {
    if (ev.data.canvas) {
        ...
        calculateGradient(context, pxWidth, pxHeight, field, alpha, rangeLimit, scale);
    }
});
...
function calculateField(point, field, rangeLimit = 10) {
  /** @type {((import("../Layers/VectorFieldLayer.js").PolarFieldPoint|import("../Layers/VectorFieldLayer.js").VectorFieldPoint)&{weight:number})[]} */
  // @ts-ignore
